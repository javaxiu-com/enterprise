...
<body class="no-skin">
					...
					<div class="row">
						<div class="col-xs-12">
								...
								<tr>
									...
									<td style="padding-top: 12px;" colspan="10" class="center">
										<h4 class="green smaller lighter">
											<b>
												<a class="btn btn-xs btn-success" title="访问网站首页" onclick="openIndex();">
													访问网站首页
												</a>
											</b>
										</h4>
							...
							<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
								<tr>
									<td style="width:150px;" class="center">
									<button class="btn btn-app btn-purple btn-xs" onclick="proHomeSingle();">
												<i class="ace-icon fa fa-print bigger-160"></i>
									</button>
									</td>
									<td style="padding-top: 12px;">
										<h4 class="green smaller lighter">生成页面(模版 6 7 8 为单页风格)</h4>
									</td>
									<td style="padding-top: 12px;" colspan="10" class="center">
										<h4 class="green smaller lighter">
											<b>
							...
							<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
								...
								<tr>
									...
									<td class="center">模版6
										<img style="cursor:pointer;" alt="模版6" src="static/images/mo6.png" width="179px"  onclick="setMoban('createHtml6')">
										<input type="radio" id="moban6" name="moban" onclick="setMoban('createHtml6')" />
									</td>
									<td class="center">模版7
										<img style="cursor:pointer;" alt="模版7" src="static/images/mo7.png" width="179px"  onclick="setMoban('createHtml7')">
										<input type="radio" id="moban7" name="moban" onclick="setMoban('createHtml7')" />
									</td>
									<td class="center">模版8
										<img style="cursor:pointer;" alt="模版8" src="static/images/mo8.png" width="179px"  onclick="setMoban('createHtml8')">
										<input type="radio" id="moban8" name="moban" onclick="setMoban('createHtml8')" />
									</td>
								</tr>
							<table>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			}else if("createHtml6" == nowMoban){
				$("#moban6").prop("checked", "checked");
				$("#no2").show();
				$("#no1").hide();
			}else if("createHtml7" == nowMoban){
				$("#moban7").prop("checked", "checked");
				$("#no2").show();
				$("#no1").hide();
			}else if("createHtml8" == nowMoban){
				$("#moban8").prop("checked", "checked");
				$("#no2").show();
				$("#no1").hide();
			}
		});
		...
		function setMoban(value){
			...
			else if("createHtml6" == value){
				$("#moban6").prop("checked", "checked");
				$("#no2").show();
				$("#no1").hide();
			}
			else if("createHtml7" == value){
				$("#moban7").prop("checked", "checked");
				$("#no2").show();
				$("#no1").hide();
			}
			else if("createHtml8" == value){
				$("#moban8").prop("checked", "checked");
				$("#no2").show();
				$("#no1").hide();
			}
			...
			$.ajax({
				...
				success: function(data){
					top.hangge();
					if("createHtml6" == value || "createHtml7" == value || "createHtml8" == value ){
						proHomeSingle(); 	//模版6 7 8是单页形式网站，只生成首页即可
					}else{
						proAll();
					}
				}
		...
		//生成模版（单页）
		function proHomeSingle(){
			top.jzts();
			$.ajax({
				type: "POST",
				url: '<%=basePath%>createHtml/proHomeSingle.do',
		    	data: {tm:new Date().getTime()},
				dataType:'json',
				cache: false,
				success: function(data){
